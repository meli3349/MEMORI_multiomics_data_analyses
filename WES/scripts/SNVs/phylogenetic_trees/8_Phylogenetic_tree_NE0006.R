...
patient_name <- "MEMORI_NE0006"
         
sample_A1 <- "MEMORI_NE0006A_T2_B1_D1"    
sample_B1 <- "MEMORI_NE0006B_T1_B1_D1"
sample_B2 <- "MEMORI_NE0006B_T3_C1_D1" 
...
neo_snv_3 <- read.table(paste0("~/analysis/WES/neoantigens/filtering_steps/tables/neoSNV_tables/", sample_B2, ".weakly_filtered_neoSNVs"), header = T, stringsAsFactors = F, sep="\t")
...
DNA_bin_driver_annotated <- cbind(Multi_mobster_output_nontail_binary$non_tail_SNV, Multi_mobster_output_nontail_binary$driver_label_Intogen, Multi_mobster_output_nontail_binary$driver_label_COSMIC_INTOGEN, Multi_mobster_output_nontail_binary$Cluster, Multi_mobster_output_nontail_binary$Info_SNV_synonymous, Multi_mobster_output_nontail_binary$NeoSNV_Bind_leve, DNAbin)
...
shared_AB1B2 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 1 & DNA_bin_driver_annotated[,label_B1] == 1 & DNA_bin_driver_annotated[,label_B2] == 1), ]

shared_AB1 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 1 & DNA_bin_driver_annotated[,label_B1] == 1 & DNA_bin_driver_annotated[,label_B2] == 0), ]

shared_AB2 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 1 & DNA_bin_driver_annotated[,label_B1] == 0 & DNA_bin_driver_annotated[,label_B2] == 1), ]

shared_B1B2 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 0 & DNA_bin_driver_annotated[,label_B1] == 1 & DNA_bin_driver_annotated[,label_B2] == 1), ]

private_A <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 1 & DNA_bin_driver_annotated[,label_B1] == 0 & DNA_bin_driver_annotated[,label_B2] == 0), ]

private_B1 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 0 & DNA_bin_driver_annotated[,label_B1] == 1 & DNA_bin_driver_annotated[,label_B2] == 0), ]

private_B2 <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 0 & DNA_bin_driver_annotated[,label_B1] == 0 & DNA_bin_driver_annotated[,label_B2] == 1), ]

View(shared_AB1B2); View(shared_AB1); View(shared_AB2); View(shared_B1B2); View(private_A); View(private_B1); View(private_B2)


# Export SNVs from tree branches according to received therapy
pre_treatment <- rbind(shared_AB1B2, private_A)
...
chemo_induced <- rbind(shared_B1B2, private_B1, private_B2)
...
pdf(paste0(fig_dir, "/", patient_name, ".phyl.tree.pdf”), height = 4.5, width = 6)
plotBS(DNAtreeMP,DNABStrees,type.tree='phylogram',bs.adj=c(1.3,1.3),label.offset=10,p=0,type="phylogram", edge.color= c('#0A9086', '#0A9086', '#E64B35FF', "grey", "#7E6148FF", "#7E6148FF"), edge.width=3,font=2,cex=1.5,tip.color=c('#E64B35FF', '#0A9086', '#0A9086', "#7E6148FF"))
