...
patient_name <- "MEMORI_RE0023"

sample_A1 <- "MEMORI_RE0023A_T1_B1_D1"       
sample_B1 <- "MEMORI_RE0023B_T2_B1_D1" 
sample_C1 <- "MEMORI_RE0023C_T1_C1_D1"  
...
Multi_mobster_output_nontail <- Multi_mobster_output_nontail[,c(1:8)]
...
present_A <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 1),]
not_present_A <- DNA_bin_driver_annotated[which(DNA_bin_driver_annotated[,label_A1] == 0),]

# Export SNVs from tree branches according to received therapy 
pre_treatment <- present_A
...
plotBS(DNAtreeMP,DNABStrees,type.tree='phylogram',bs.adj=c(1.3,1.3),label.offset=15,p=0,type="phylogram", edge.color= c('#E64B35FF', '#0A9086', '#568ebd', 'grey', "#7E6148FF", "#7E6148FF"), edge.width=3,font=2,cex=1.5,tip.color=c('#E64B35FF', '#0A9086', '#568ebd', "#7E6148FF"))
add.scale.bar(440,01, lwd=3,font=2);mtext(paste0('HI: ',hi),side=3,font=2,cex=1.25,ask=T);show.node.label = T;
